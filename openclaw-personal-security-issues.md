# 🦞 OpenClaw Is a Personal Security Nightmare — Here's What You Need to Know
...
- It executed a silent `curl` command that **exfiltrated data** to an external server controlled by the skill author — **without any user notification**
...
- Grabbed by **infostealer malware** (The Hacker News reported a specific infostealer campaign targeting OpenClaw configuration files and gateway tokens)
...
The breakneck speed of OpenClaw's rise attracted scammers immediately. Taking advantage of the rapid name changes:
...
The **Autoriteit Persoonsgegevens (AP)** — the Netherlands' official data protection authority — issued a formal warning against using OpenClaw and similar experimental autonomous AI systems. Their concern:
...
*Published on the Daily Blog by acekapila-git | Research-backed, no fluff.*
